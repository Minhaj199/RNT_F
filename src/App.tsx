...
const App = () => {
  ...
  const loadNodes = async () => {
    try {
      setLoading(true);
      const result: { data: NodeData[] } = await request({ url: '/api/fetch-nodes' });
  ...
  const handleCreateNode = async (nodeData: Partial<NodeData>) => {
    try {
      ...
      enqueueSnackbar(err instanceof Error ? err.message : 'Failed to create node', { variant: 'error' });
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">Loading tree structure...</div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-6 max-w-4xl">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">Infinite nesting with persistent storage</p>
              </div>
            </div>
            <button
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          {rootNodes.length === 0 ? (
            <div className="text-center py-12 text-gray-500">No nodes yet</div>
          ) : (
            rootNodes.map((node) => (
              <TreeNode
                key={node._id}
